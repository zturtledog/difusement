...
public class obsidian_pedestal_ber implements BlockEntityRenderer<obsidian_pedestal_be> {
    ...
    public void render(obsidian_pedestal_be entity, float delta, PoseStack pose, MultiBufferSource bufffer,
        ...
        pose.pushPose();
        float bob = (ease(Math.abs(((entity.time + entity.timeoff) % 50) / 50.0f - 0.5f) * 2f) - 0.5f) / 6f;
        pose.translate(0.5, 1.5 + bob, 0.5);
        if (!entity.item.getItem().getClass().isAssignableFrom(BlockItem.class)) {
            pose.scale(0.75f, 0.75f, 0.75f);
        }
        pose.mulPose(Quaternion.fromXYZDegrees(new Vector3f(bob / 10f, (entity.time + entity.timeoff), bob / 10f)));
